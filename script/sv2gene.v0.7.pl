...
my (%genes, %features, %exons, %transcript_length, %feature_lengths);
...
while(<$bed_in>){
  ...
  if ($feature eq 'gene'){
    ...
    $feature_lengths{$feature} += $feature_length;
    if ($feature eq 'exon'){
...
my $genome_length = '137547960';
my $non_feature = 0;

for (keys %feature_lengths) {
  my $length = $feature_lengths{$_};
  $non_feature += $feature_lengths{$_};
  print join (", ", $_, $length, $length/$genome_length) . "\n";
}

my $intergenic = ($genome_length - $non_feature);
