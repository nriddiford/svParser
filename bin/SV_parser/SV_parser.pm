...
sub parse {
    ...
    while (<$in>) {
        ...
        my (
            $chr,          $start,         $id,   $ref,
            $alt,          $quality_score, $filt, $info_block,
            $format_block, @sample_info
        ) = @fields;

        my %sample_parts;

        push @{ $sample_parts{ $samples[$_] } }, split( /:/, $sample_info[$_] )
          for 0 .. $#samples;
        ...
        for my $sample (@samples) {
            ...
            for my $info ( 0 .. $#format ) {
                $sample_info{$id}{$sample}{ $format[$info] } =
                  $sample_parts{$sample}[$info];
            }
        ...
        for my $normal (@normals) {
            if (   $sample_info{$id}{$normal}{'GT'} eq '1/1'
                or $sample_info{$id}{$normal}{'GT'} eq '0/1' )
            {
                push @filter_reasons,
                  "$normal\_not_homo_ref=" . $sample_info{$id}{$normal}{'GT'};
            }
        }
        ...
        if ( $type eq 'lumpy' ) {
            ( $SV_length, $chr2, $stop, $t_SR, $t_PE, $filters ) = lumpy(
                $id,          $info_block,   $SV_type,
                $alt,         $start,        \%sample_info,
                $tumour_name, $control_name, \@samples,
                \@normals,    \@filter_reasons
            );
        }

        elsif ( $type eq 'delly' ) {
            ( $SV_length, $chr2, $stop, $t_SR, $t_PE, $filters ) =
              delly( $info_block, $start, $SV_type, \@filter_reasons );
        }

        $filters = chrom_filter( $chr, $chr2, $filters );

        $SVs{$id} = [
            @fields[ 0 .. 10 ], $SV_type, $SV_length,
            $stop,              $chr2,    $t_SR,
            $t_PE,              $filters, \@samples
        ];

        $info{$id} = [
            [@format],       [%format_long],
            [%info_long],    [@tumour_parts],
            [@normal_parts], [%information],
            [%sample_info]
        ];
...
sub print_variants {
    ...
    for ( sort { $a <=> $b } keys %filtered_SVs ) {
        ...
        if ( $line =~ /^#/ ) {
            ...
            my @cols = split( "\t", $line );
            print $out
              join( "\t", @cols[ 0 .. 5 ], "PASS", @cols[ 7 .. $#cols ] )
              . "\n";
        }
    }
...
sub lumpy {
    my (
        $id,      $info_block,  $SV_type, $alt,
        $start,   $sample_info, $tumour,  $control,
        $samples, $normals,     $filters
    ) = @_;
    ...
    my ( $t_PE, $t_SR, $all_c_PE, $all_c_SR, $c_PE, $c_SR ) =
      ( 0, 0, 0, 0, 0, 0 );
...
# We want to be strict, so include all controls used for genotyping (and sum read support)
        for my $normal (@normals) {
            $sample_info{$id}{$normal}{'PE'} eq '.'
              ? $sample_info{$id}{$normal}{'PE'} = '0'
              : $all_c_PE += $sample_info{$id}{$normal}{'PE'};
            $sample_info{$id}{$normal}{'SR'} eq '.'
              ? $sample_info{$id}{$normal}{'SR'} = '0'
              : $all_c_SR += $sample_info{$id}{$normal}{'SR'};
        }
        ...
        if ( $all_control_read_support > 0 ) {
            push @filter_reasons, 'precise_var_with_normal_read_support='
              . $all_control_read_support;
        }
    }
    ...
    if ( $info_block =~ /IMPRECISE;/ ) {
        ...
        my $sc_direct_control_read_support =
          $direct_control_read_support + 0.001;

        if ( $direct_control_read_support > 1 ) {
            push @filter_reasons, "imprecise_var_with_$control\_read_support>1="
              . $direct_control_read_support;
        }
    ...
    if ( exists $sample_info{$id}{$tumour}{'SQ'} ) {
        $sample_info{$id}{$tumour}{'SQ'} = 0
          if $sample_info{$id}{$tumour}{'SQ'} eq '.';
...
sub summarise_variants {
    ...
    for ( keys %{$SVs} ) {
        ...
        my (
            $chr,               $start,             $id,
            $ref,               $alt,               $quality_score,
            $filt,              $info_block,        $format_block,
            $tumour_info_block, $normal_info_block, $sv_type,
            $SV_length,         $stop,              $chr2,
            $SR,                $PE,                $filters
        ) = @{ $SVs->{$_} };
    ...
    if ($filter_flag) {
        ...
        say " - $_: $filtered_sv{$_}"
          for sort { $filtered_sv{$b} <=> $filtered_sv{$a} } keys %filtered_sv;
        print "\n";
    }
    ...
    for (
        sort { $support_by_chrom{$b}[0] <=> $support_by_chrom{$a}[0] }
        keys %support_by_chrom
      )
    {
        $top_count++;
        print join( "\n",
...
sub dump_variants {
    ...
    if ( $chromosome =~ /:/ ) {
        ...
        say
"Limiting search to SVs within region '$query_start-$query_stop' on chromosome '$chromosome'";

        $specified_region = 1;
    }

    say "Enter any key to start cycling through calls or enter 'q' to exit";
    say "Running in filter mode - not displaying filtered calls"
      if $filter_flag;
      ...
      )
    {
        ...
        if ($specified_region) {
            ...
            if (    ( $start < $query_start or $start > $query_stop )
                and ( $stop < $query_stop or $stop > $query_stop ) )
            {
                next;
            }
        ...
        my $next_line = <>;
        say
"Displaying info for variant '$id'. Enter any key to go to the next variant or type 'q' to exit\n";
